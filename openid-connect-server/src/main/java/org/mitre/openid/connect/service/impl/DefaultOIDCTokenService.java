...
public class DefaultOIDCTokenService implements OIDCTokenService {
	...
	private OAuth2AccessTokenEntity createAssociatedToken(ClientDetailsEntity client, Set<String> scope) {
		...
		JWSAlgorithm signingAlg = jwtService.getDefaultSigningAlgorithm();
		System.out.println("jwtService.getDefaultSignerKeyId() "+jwtService.getDefaultSignerKeyId());
		JWSHeader header = new JWSHeader(signingAlg, null, null, null, null, null, null, null, null, null,
				jwtService.getDefaultSignerKeyId(),
				null, null);
		...
		System.out.println("DefaultOIDCTokenService.createAssociatedToken before setJWT ");
		token.setJwt(signed);
